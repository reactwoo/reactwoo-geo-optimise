...
<section class="rwgo-tracking-guidance" aria-labelledby="rwgo-tracking-guidance-title">
	...
	<div class="rwgo-grid rwgo-grid--2">
		<div class="rwgo-panel rwgo-panel--compact">
			...
			<p class="rwgo-setting-row__hint"><?php esc_html_e( 'The front-end script binds elements that include data-rwgo-experiment-key, data-rwgo-goal-id, and data-rwgo-handler-id. Optional: data-rwgo-variant-id (otherwise the resolved variant from the server is used), data-rwgo-element-fingerprint, data-rwgo-element-key (stable semantic key across Control/Variant). Under Settings → Strict binding, a non-empty fingerprint is required on stamped elements; automatic page-view goals are skipped in that mode. Matching goals are also sent to the REST API so they appear in Reports (see Developer → Browser REST endpoint). If clicks fire but conversions do not appear, open the browser developer console on the test page: failed POST /rwgo/v1/goal responses log as [RWGO] Goal REST rejected with the WordPress error code (e.g. rwgo_bad_goal, rwgo_invalid_nonce).', 'reactwoo-geo-optimise' ); ?></p>
		</div>
		<div class="rwgo-panel rwgo-panel--compact">
