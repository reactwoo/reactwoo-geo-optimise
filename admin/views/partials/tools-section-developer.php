...
<div class="rwgo-stack rwgo-stack--developer">
	<div class="rwgc-card rwgo-resync-card">
		<h2><?php esc_html_e( 'Resync imported or staged tests', 'reactwoo-geo-optimise' ); ?></h2>
		<p><?php esc_html_e( 'Run both safe repair tasks together after a site clone, SQL import, front-page change, or builder content replacement. This will re-resolve stored Control and Variant page bindings, then refresh saved goal and handler IDs from live Elementor or Gutenberg content.', 'reactwoo-geo-optimise' ); ?></p>
		<ul>
			<li><?php esc_html_e( 'Page bindings: repairs stored source and variant page IDs when slugs or URL paths still match this environment.', 'reactwoo-geo-optimise' ); ?></li>
			<li><?php esc_html_e( 'Goal IDs: refreshes saved builder goal_id and handler_id pairs when CTA or form widgets were recreated, duplicated, or imported.', 'reactwoo-geo-optimise' ); ?></li>
		</ul>
		<p class="description"><?php esc_html_e( 'Both operations are non-destructive: they only refresh saved references from live content and page lookups.', 'reactwoo-geo-optimise' ); ?></p>
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="rwgo-cta-row">
			<input type="hidden" name="action" value="rwgo_resync_all_safe" />
			<?php wp_nonce_field( 'rwgo_resync_all_safe' ); ?>
			<?php submit_button( __( 'Run full safe resync', 'reactwoo-geo-optimise' ), 'primary', 'submit', false, array( 'class' => 'button button-primary rwgo-btn rwgo-btn--primary' ) ); ?>
		</form>
		<?php if ( isset( $_GET['rwgo_resync_all'] ) || isset( $_GET['rwgo_resynced'] ) || isset( $_GET['rwgo_goal_resync'] ) ) : // phpcs:ignore WordPress.Security.NonceVerification.Recommended ?>
			<?php
			...
			<div class="rwgo-hint rwgo-hint--accent">
				<p><strong><?php esc_html_e( 'Page bindings:', 'reactwoo-geo-optimise' ); ?></strong> <?php echo esc_html( sprintf( /* translators: 1: scanned count, 2: updated count */ __( '%1$d tests scanned, %2$d updated.', 'reactwoo-geo-optimise' ), $rwgo_rs_sc, $rwgo_rs_upd ) ); ?></p>
				<?php if ( $rwgo_rs_src > 0 || $rwgo_rs_var > 0 ) : ?>
					<p><?php echo esc_html( sprintf( /* translators: 1: source repairs, 2: variant repairs */ __( 'Source repairs: %1$d. Variant repairs: %2$d.', 'reactwoo-geo-optimise' ), $rwgo_rs_src, $rwgo_rs_var ) ); ?></p>
				<?php endif; ?>
				<?php if ( $rwgo_rs_for > 0 ) : ?>
					<p><?php echo esc_html( sprintf( /* translators: %d: forced front-page repairs */ __( 'Forced front-page source repairs: %d.', 'reactwoo-geo-optimise' ), $rwgo_rs_for ) ); ?></p>
				<?php endif; ?>
				<p><strong><?php esc_html_e( 'Goal IDs:', 'reactwoo-geo-optimise' ); ?></strong> <?php echo esc_html( sprintf( /* translators: 1: updated tests, 2: scanned tests */ __( '%1$d test(s) updated, scanned %2$d.', 'reactwoo-geo-optimise' ), $rwgo_gr_upd, $rwgo_gr_sc ) ); ?></p>
				<?php if ( $rwgo_gr_pt > 0 ) : ?>
					<p><?php echo esc_html( sprintf( /* translators: %d: goal rows patched */ __( 'Goal rows refreshed: %d.', 'reactwoo-geo-optimise' ), $rwgo_gr_pt ) ); ?></p>
				<?php endif; ?>
			</div>
		<?php endif; ?>
	</div>
