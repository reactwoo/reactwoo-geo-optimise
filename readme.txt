...
= 0.4.94 =
* **Fix:** Stop calling removed Geo Core `RWGC_Elementor_Ajax::is_heavy_elementor_ajax()` (fatal on admin/ajax when Core no longer classifies heavy Elementor AJAX).
* **Fix:** Compact AI snapshot UVP when the analyzer stores `{ text, confidence }` instead of a string.
...
= 0.4.88 =
* **AI Connector diagnostics:** Settings panel to test local deterministic review, remote API auth, remote workflow (Pro gate), and remote→local fallback separately; execution mode control restored after Advanced redirect into Optimise Settings.
...
= 0.4.73 =
* **Measurement / Atomic write:** Elementor document writer + measurement stamper sync Control→Variant B element keys (V3/V4); auto on variant duplicate; Developer Tools sync action; Atomic widget goal support.

= 0.4.72 =
* **AI Review UX:** Chat-first hierarchy — review types before target controls, Refine setup collapsed by default, Full review as select-all, compact results summary, hidden target-field CSS fix, legacy Experiments/Reports removed from Experiences section strip.

= 0.4.71 =
* **Measurement:** First-class `experiment_exposure` events (session/day-deduped), tracking manifest on front-end config, and `rwgo_experiment_exposure` dataLayer push.

= 0.4.70 =
* **Generation router:** migrate remaining workflows (opportunity review, copy implement, competitor research, intelligence, weather facets) onto the shared transport router.

= 0.4.69 =
* **Measurement:** Stable `data-rwgo-element-key` on Elementor/Gutenberg goals; passed through tracking, REST payloads, and GTM dataLayer docs.
...
= 0.4.63 =
* **Geo AI snapshot:** Append `geo_optimise` experiment metadata and experiment→page relationships to Geo Core site intelligence snapshots.

= 0.4.62 =
* **Geo AI handoff:** Create Test reads `rwgo_prefill_*` query args from Geo AI intelligence runs; suite handoff banner on Create Test wizard.

= 0.4.61 =
* **i18n:** Queue textdomain via Geo Core `RWGC_I18n` on `plugins_loaded` priority 6 (WP 6.7 JIT fix with Geo Core 1.8.29).

= 0.4.60 =
* **Suite release:** Aligned with Geo Core 1.7.9 contextual admin shell (integration categories, scoped Experiences hub).
...
= 0.4.57 =
* **Targeting:** Experiment reports route under Geo platform **Targeting** section (was Insights).
