...
class RWGA_Weather_Facet_Suggester {
	...
	public static function suggest_for_product( $product_id ) {
		...
		if ( class_exists( 'RWGA_Generation_Router', false ) ) {
			...
			if ( in_array( $mode, array( 'managed', 'remote', 'wordpress_ai' ), true ) && is_wp_error( $remote ) ) {
				return array(
					'facets' => array(),
					'scores' => array(),
					'source' => 'remote_error',
	...
	private static function suggest_via_remote( $product_id ) {
		...
		$envelope = RWGA_Generation_Router::generate(
			self::REMOTE_WORKFLOW_KEY,
			array(
				'payload'        => $payload,
				'local_callback' => static function ( $workflow_key, array $request ) use ( $product_id ) {
					unset( $workflow_key, $request );
					$local = self::suggest_via_keywords( $product_id );
					return array(
						'facets'    => isset( $local['facets'] ) ? $local['facets'] : array(),
						'scores'    => isset( $local['scores'] ) ? $local['scores'] : array(),
						'rationale' => isset( $local['rationale'] ) ? $local['rationale'] : '',
					);
				},
			)
		);
		...
		$response = isset( $envelope['engine_response'] ) && is_array( $envelope['engine_response'] )
		...
		$transport = isset( $envelope['transport'] ) ? sanitize_key( (string) $envelope['transport'] ) : 'managed';
		$out       = array(
			'facets' => $facets,
			'scores' => $scores,
			'source' => ( 'local' === $transport ) ? 'local' : ( ( 'wordpress_ai' === $transport ) ? 'wordpress_ai' : 'remote' ),
		);
